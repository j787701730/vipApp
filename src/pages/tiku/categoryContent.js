...
export default class Index extends Component {
  ...
  _getData = () => {
    ...
    ajax(`https://apicloud.mob.com/tiku/shitiku/query?page=${this.state.page}&size=1&cid=${this.$router.params.cid}`,
      '', false, (data) => {
      Taro.hideLoading();
      if (data.retCode == 200) {
            ...
            <View style={{marginBottom: '6px'}}>
              {result.list[0].tikuType === 'select'
                ? result.list[0].val.length == 1
                  ...
                  :
                  <Text>{`正确答案：${result && result.list[0].val.indexOf('1') > -1 
                    ? 'A' : ''}${result && result.list[0].val.indexOf('2') > -1
                    ? 'B' : ''}${result && result.list[0].val.indexOf('3') > -1 
                    ? 'C' : ''}${result && result.list[0].val.indexOf('4') > -1 
                    ? 'D' : ''}`}</Text>
